...
function mdv_recent_posts( $no_posts = 10, $before = '<li>', $after = '</li>', $hide_pass_post = true, $skip_posts = 0, $show_excerpts = false, $include_pages = false ) {
	global $wpdb;

	$time_difference = get_settings( 'gmt_offset' );
	$now = gmdate( "Y-m-d H:i:s", time() );
	$request = "SELECT ID, post_title, post_excerpt, post_date, comment_count FROM $wpdb->posts WHERE post_status = 'publish' ";
	if ( $hide_pass_post )
	...
	if ( $posts ) {
		foreach ( $posts as $post ) {
			...
			$output .= $before . '<span class="date">'.$date.'</span><a href="' . esc_url( $permalink ) . '" rel="bookmark" title="Permanent Link: ' . esc_attr( $post_title ) . '">' . esc_html( $post_title ) . '</a><span class="comments">('.$post->comment_count.') Comments</span>';
			if ( $show_excerpts ) {
...
function mdv_recent_comments($no_comments = 5, $comment_lenth = 6, $before = '<li>', $after = '</li>', $show_pass_post = false, $comment_style = 0) {
    global $wpdb;
	...
	if ($comments) {
		foreach ($comments as $comment) {
			...
			if ($comment_style == 1) {
				...
				$output .= $before . '<span>' . $comment_author .'</span> <a href="' . $permalink;
...
function twentyten_setup() {
	...
	define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'twentyten_header_image_height', 174 ) );
