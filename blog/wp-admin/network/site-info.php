...
<form method="post" action="site-info.php?action=update-site">
	...
	<table class="form-table">
		<tr class="form-field form-required">
			...
			if ( $is_main_site ) { ?>
			<td><code><?=$protocol; echo esc_attr( $details->domain ) ?></code></td>
			<?php } else { ?>
			<td><?=$protocol;?><input name="blog[domain]" type="text" id="domain" value="<?=esc_attr( $details->domain ) ?>" size="33" /></td>
			<?php } ?>
		</tr>
		<tr class="form-field form-required">
		...
		<tr>
			...
			<?php foreach ( $attribute_fields as $field_key => $field_label ) : ?>
				<label><input type="checkbox" name="blog[<?=$field_key;?>]" value="1" <?php checked( (bool) $details->$field_key, true ); disabled( ! in_array( $details->$field_key, array( 0, 1 ) ) );?> />
