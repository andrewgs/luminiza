...
<a href="themes.php" class="nav-tab nav-tab-active"><?=esc_html( $title );?></a><a href="<?=admin_url( 'theme-install.php');?>" class="nav-tab"><?=esc_html_x('Install Themes', 'theme');?></a>
<?php else : ?>
<h2><?=esc_html( $title );?>
<?php endif;?>
</h2>
...
<img src="<?=$ct->theme_root_uri . '/' . $ct->stylesheet . '/' . $ct->screenshot;?>" alt="<?php _e('Current theme preview');?>" />
<?php endif;?>
<h4><?php
	/* translators: 1: theme title, 2: theme version, 3: theme author */
	printf(__('%1$s %2$s by %3$s'), $ct->title, $ct->version, $ct->author) ;?></h4>
<p class="theme-description"><?=$ct->description;?></p>
...
<?php $feature_list = get_theme_feature_list();?>
		...
		<div class="feature-container">
			...
			<ol class="feature-group">
				<?php foreach ( $features as $key => $feature ) :
						...
						?>
				<li>
					<input type="checkbox" name="features[]" id="feature-id-<?=$key;?>" value="<?=$key;?>" <?php checked( in_array( $key, $wp_list_table->features ) );?>/>
					<label for="feature-id-<?=$key;?>"><?=$feature_name;?></label>
				</li>
				<?php endforeach;?>
			</ol>
		</div>
	...
	<div class="feature-container">
		...
		&nbsp;
		<small><a id="mini-filter-click" href="<?=esc_url( remove_query_arg( array('filter', 'features', 'submit') ) );?>"><?php _e( 'Close filters' )?></a></small>
	</div>
	<br/>
	</div>
	<br class="clear"/>
