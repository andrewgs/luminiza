...
<?php foreach ( $post_formats[0] as $format ): ?>
		<option<?php selected( get_option('default_post_format'), $format );?> value="<?=esc_attr( $format );?>"><?=esc_html( get_post_format_string( $format ) );?></option>
<?php endforeach;?>
	</select>
</td>
</tr>
...
<p class="pressthis"><a onclick="return false;" oncontextmenu="if(window.navigator.userAgent.indexOf('WebKit')!=-1)jQuery('.pressthis-code').show().find('textarea').focus().select();return false;" href="<?=htmlspecialchars( get_shortcut_link() );?>"><span><?php _e('Press This') ?></span></a></p>
<div class="pressthis-code" style="display:none;">
	...
	<p><textarea rows="5" cols="120" readonly="readonly"><?=htmlspecialchars( get_shortcut_link() );?></textarea></p>
</div>
...
<textarea name="ping_sites" id="ping_sites" class="large-text code" rows="3"><?=esc_textarea( get_option('ping_sites') );?></textarea>
